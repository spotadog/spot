...
## Popup seen tracking — prompt 037

- Request: recognize traditional browser popups and website overlays, associate stable identities where possible and integrate repeat appearances with URL tracking. Complete prompt saved before implementation.
- Changes: browser popup/opener-tab classification, independent visible-dialog/overlay detector, item/activating-link/dialog/fingerprint identities and local idempotent appearance storage. Shared popup/sidebar history shows First seen/Seen before and counts, with the existing toggle, session/all-time modes and combined clear action. URL counts, matching and AI remain separate; no new permissions or external data flow.
- Validation: final npm run check passed 157 unit tests, build and the complete automated MV3 suite. Coverage includes detection/identity, wrappers, appearance lifecycle, disabled behavior, source validation, retries, concurrency, persistence, clearing and actual browser popup/overlay reopenings with Seen before UI checks. Corrected HTTP content-token generation and nested-wrapper detection during testing. git diff --check passed.
- Limits/next step: heuristic detection is not proof of reading; generic IDs, dynamic fallback content, unmarked/iframe/shadow-root overlays and native browser alerts have documented limits. Reload dist/, enable URL tracking and inspect Popup history while reopening representative site popups. No manual/live-site testing or external AI calls.
- Publication: existing main, no new branch, SSH identity from prompt 002; origin fetched without divergence. Invalid GitHub CLI authentication uses the documented repository/commit review-note fallback.

## Autoplay location checkbot — prompt 038

- Request: validate autoplay keyword stops against uploaded profile locations using the supplied scout JSON structure. Complete prompt preserved before implementation.
- Changes: optional Settings location filter, scout JSON upload/clear, strict validation, atomic local replacement and persistence, minimal scanning projection and immediate broadcasts. Explicit post/card authors must have a checked accountBasedIn matching the configured location before positive/color-eligible pauses. Unknown/nonmatching authors continue; slowdown remains independent. Pure matching and storage/AI boundaries retained.
- Validation: npm run check passed 163 unit tests, build and full automated MV3 browser suite. Supplied sample validated with all 377 records; synthetic fixtures avoid committing personal data. git diff --check passed. Browser fixture initially waited incorrectly; fixed to wait for verified-pause status and reran successfully.
