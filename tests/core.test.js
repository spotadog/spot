...
test('legacy preferences gain popup default without changing profiles, model or key', async () => {
  ...
  const store = createStore(area);
  assert.deepEqual(await store.read(), { ...saved, preferences: { model: 'custom-model', sidebar: false, tracking: false, provider: 'openai', providerModels: { openai: 'custom-model' } } });
