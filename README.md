...
- Optional [URL visit tracking](#url-visit-tracking), with separate clearable session and all-time histories.
...
Enable **Track URL visits** in the popup or sidebar to count new top-level HTTP/HTTPS navigations across tabs, independently of highlighting and keyword counting. Tracking defaults off. Enabling it starts with subsequent navigations; it does not import browser history or count already open tabs. Reloads, back/forward navigations and URL-changing History API/fragment updates count; tab switching, subframes, redirects before a committed page do not. Same-URL History API state updates are ignored once that tab’s URL has been recorded in the selected history; its first observed route event counts if no earlier navigation is known. URLs include their query and fragment, so different URLs have separate counts.

Choose **Per session** for the current browser session (survives service-worker sleep, resets on browser restart or extension reload), or **All time** for persistence across sessions. Each mode has its own history; switching modes preserves both and records future visits only in the selected mode. Turning tracking off retains recorded history without recording further visits. The toggle and selected mode persist.

The current tab shows its recorded visit count. When that number changes, only the number briefly pops; initial display and unchanged refreshes stay still. Reduced-motion preferences disable the animation. Expand **URL visit history** to browse or search URLs and counts; **Show more** reveals additional entries. **Clear URL history** asks for confirmation and clears both modes, including while tracking is off. If tracking remains on, subsequent visits start counting again. Clearing URL history also clears popup history, but does not clear keyword counts, profiles or Chrome's own history.

URL strings and counts stay locally in your browser, separate from profile backups and AI requests. History has no automatic eviction; browser storage limits apply. A failed write preserves saved counts and reports an error to an open view. Reload the extension from `dist/` after building and accept Chrome's added `webNavigation` permission if prompted. Navigation events use Chrome's [webNavigation API](https://developer.chrome.com/docs/extensions/reference/api/webNavigation).

### Popup seen history

**Track URL visits** also enables **Popup history**, shown inside URL visit history in the sidebar or popup editor. It shares the selected session/all-time mode and clear action. Rows show **First seen** or **Seen before** and an appearance count. Popup counts are separate from URL navigation counts: an overlay does not add a URL visit. All data stays local and is excluded from profile backups and AI submissions.

Browser popup windows and tabs with an opener are recognized on committed HTTP/HTTPS navigation, using the destination URL as identity. Reloads count as another appearance; new tabs opened by links are included and labeled **Browser popup / new tab**. A browser navigation indicates that content opened, not proof that the user read it. Blocked windows, browser-native alerts/confirms, and windows that remain on about:blank or other non-web URLs cannot be identified this way.

On visible top-level webpages, the detector recognizes visible native dialogs/popovers, ARIA dialogs/modal elements, and positioned, raised elements with modal/popup/overlay class names. It avoids counting a backdrop and its nested dialog separately. A continuously visible identified overlay counts once; closing/reopening or changing a supplied item identifier counts again. Detection is debounced, with a one-second fallback for CSS-only visibility changes. Enabling tracking detects currently visible overlays; disabling disconnects observation. Clearing history or switching modes does not count an already acknowledged visible overlay again; its next appearance is recorded.

Identity uses site-scoped explicit data-item-id/data-popup-id first, a recently clicked activating HTTP/HTTPS link next, then a page-scoped dialog ID/accessibility label/heading. Otherwise a bounded content fingerprint is computed locally; raw fallback text is not persisted. URL identities include query/fragment. Generic reused dialog IDs can merge unrelated items, changing fallback content can split identities, and delayed/unrelated links may not identify an overlay reliably. Detection is best effort: unmarked overlays, shadow DOM, iframe contents, very short appearances and unusual CSS layouts may be missed. Hidden/background documents are not counted until visible. No new permission is needed beyond existing page access and webNavigation.
...
Choose a profile from **Profile** to view its details. Switch between **Positive Words** and **Negative Words** using the tabs; only the selected list is displayed, expanding with normal page scrolling. Arrow Left/Right, Home and End select tabs; Tab moves into the selected panel. Switching tabs preserves drafts and search. Profile details are read-only by default; keyword controls remain available. **Search keywords** filters both keyword groups immediately using case-insensitive partial matches; clearing it restores every row. Switching profiles resets search. Each keyword checkbox shows activity: checked means active, unchecked means inactive. Activity changes save immediately for existing profiles.

Choose the profile **Edit** action to change its name and enabled state, then **Save profile** or **Cancel**. These controls apply only to profile details. Keywords are always individually editable: **Add Keyword** creates a row in the selected group; **Edit** opens one keyword; **Save keyword** immediately persists its text, matching criterion and color without saving profile details or other unfinished rows. **Cancel** restores that row’s previous value or discards a new row. **Remove** asks for confirmation and immediately persists removal. Activity checkboxes save immediately. Failed keyword writes retain the row edit for retry. Profile Save and Cancel preserve unfinished keyword edits and already saved keyword changes. Switching profiles or starting another profile asks before discarding unsaved edits; closing the popup/page discards them. Background updates retain open drafts.

For a new profile, its initial name and keyword rows are created together with **Save profile**. Once created, every keyword save is independent.

**Highlight color** offers six presets (Yellow, Red, Green, Blue, Purple, Orange) and a native **Custom color** picker when adding or editing a keyword. The selected preset is outlined and the selected name/hex value is displayed. Save keyword persists the color and refreshes highlights for an existing profile. Cancel discards color edits. Colors survive reopening and profile backup round trips. Legacy strings and records without `color` retain yellow positive/red negative defaults, without a migration. Optional `color` uses six-digit `#rrggbb` hex; writes validate and normalize case, and malformed stored colors render with the group default. Custom backgrounds use black or white text for contrast. Negative overlap precedence is independent of color. Identical same-kind match intervals use the last contributing keyword in profile/list order; other same-kind overlaps use CSS highlight registration order. Color changes do not reset keyword counts. Older extension versions cannot import records with color fields.
...
Open **Settings → Location checkbot**, upload an X Profile Scout JSON file, enter **Account based in** (for example, `United States`), enable **Verify location before keyword pauses**, and click **Save location check**. Also enable **Pause after positive keyword** in the Auto Scroll controls. The checkbot filters existing positive/color-eligible pauses at the configured eyeball level. It compares the post author's uploaded `accountBasedIn` with the chosen location, ignoring case and repeated whitespace but requiring a whole-value match. Connection source, follow status and notes do not affect the decision. A checked matching author pauses; missing, unavailable, ambiguous or nonmatching authors keep scrolling. Slowdown, manual pause, Resume and end-of-content safeguards retain their behavior. Disabling the checkbot restores ordinary keyword pausing.

Uploads follow the supplied scout format exactly: `{ "version": 1, "exportedAt": "2026-09-11T11:38:35.415Z", "records": [...] }`. Each record contains string fields `accountBasedIn`, `checkedAt`, `connectedVia`, `dateJoined`, `followStatus`, `handle`, `note`, `profileUrl` and `status` (`checked` or `unavailable`). Timestamps must parse as dates; handles must agree with their X/Twitter profile URLs. All sample fields are retained. Unsupported versions, unknown/missing fields, duplicate handles, invalid records and files over 5 MiB or 10,000 records are rejected before saving. Uploads replace the whole previous scout dataset atomically; an empty records array or **Clear uploaded profiles** removes it. This is a separate format from Spot a Dog keyword-profile backups.

The dataset and location setting persist locally through the storage adapter, outside keyword-profile backups. Successful changes refresh open content scripts immediately; failed imports retain the previous data. Content scripts receive only the enabled flag, target location and eligible handles, not notes or the full export. No AI, account-following or network lookup is performed. Uploaded records are evidence as of their export, not live verification. Clear or replace stale data as needed.

Author identification supports X-style post author headers (`User-Name`) in articles and profile cards (`UserCell`), plus explicit `rel="author"` links in articles. It excludes quoted headers and never uses arbitrary mentions or links in post text. Unsupported layouts or absent author metadata are treated as unknown. Reload the extension from `dist/`, then manually check representative X feeds/cards after uploading your export; live X layout compatibility is not guaranteed by the local fixtures.
...
- The UI uses **Save profile** for name/enabled edits and independent **Save keyword** actions for existing profiles. Row mutations operate on the latest stored profile; a changed or removed row is rejected with recovery guidance instead of overwriting it. Profile-detail saves preserve stored keywords. Unsaved edits are lost when the page closes. Operations are serialized by the service worker; concurrent edits to profile details retain last-save behavior.
...
This includes page URLs, scanned text, matching results, counts, and URL/context fingerprints derived from browsing. Optional counting passes URLs and normalized matching text contexts from the content script to the extension's background worker through local Chrome messages. The worker is part of your browser, not a backend server; it hashes and stores keyword history locally. Optional URL visit tracking separately stores readable URLs and visit counts locally for the history view. No analytics, telemetry, remote error reporting, developer backend, or cloud synchronization receives browsing data.
...
In **Settings → Automatic ad skipping**, turn on **Automatically skip video ads**. It defaults off, saves automatically for all tabs and survives browser restarts. It operates independently of keyword highlighting, location checks and Auto Scroll. Turn it off to cancel pending checks immediately.

Before matching, the extension identifies the largest prominent visible HTML video and requires it to be playing, with available frame data and playback time greater than zero. Prominence is a heuristic: at least 300 × 160 visible pixels and 15% of the viewport area. A paused main player does not qualify just because a smaller preview is playing. Controls must share a compact player container with that video and sit over or immediately beside it.

Labels include **Skip ad**, **Skip ads**, **Skip this ad**, **Skip advertisement**, **Skip advert** and **Skip commercial**, including “the/all,” “now,” whitespace/case differences and trailing arrow/punctuation variants. The matcher checks visible text/input value, ARIA labels, referenced labels and titles. Custom controls with ad-skip IDs/classes/test IDs are supported; a bare **Skip** or icon also needs an ad-skip identifier or a recognized ad-playing player state. **Skip intro**, unrelated page buttons and countdown text remain excluded. No countdown is bypassed.

MGP-based video players use a custom ad-roll control. The extension recognizes the rendered `mgp_adRollSkipButton` and waits for its `mgp_skippable` class within an active ad-roll container, then sends the desktop `mouseup` activation that this player handles. Ordinary buttons retain their normal click action. Detection uses player markup and readiness, with no hostname restriction. Compatibility was checked against the shared MGP 9.2.1 script loaded by both reported sites and reproduced on separate synthetic domains; live served ads were not verified.

Controls must be enabled, visible, within the viewport and unobstructed at a hit-test point. DOM changes and a 500 ms fallback detect new controls, playback and eligibility changes. A continuously eligible button is clicked once, including across a playback pause/resume; an observed disappearance while the video is playing or toggling the feature off/on rearms it. Nested skip-label elements do not cause duplicate clicks.

Checks run locally in visible documents, with no AI or external service calls, new permissions or persistent page history. The current content-script scope covers the top document, not iframe contents or shadow roots. Sites requiring trusted physical input may ignore the programmatic click. Main-content classification and player markup are heuristics, not guaranteed site compatibility. Manual check: reload `dist/`, reopen Settings, enable the setting and play a prominent video; verify its available ad-skip button and that pausing the video or disabling the setting prevents new clicks.
