...
YouPorn's MGP player uses a custom ad-roll control. The extension waits for its `skippable` class within an active ad-roll container, then sends the desktop `mouseup` activation that this player handles. Ordinary buttons retain their normal click action. This compatibility fix is based on inspection of the site's loaded MGP 9.2.1 player code and reproduced in local media fixtures; a live served ad was not verified.
