...
The current tab shows its recorded visit count. When that number changes, only the number briefly pops; initial display and unchanged refreshes stay still. Reduced-motion preferences disable the animation. Expand **URL visit history** to browse or search URLs and counts; **Show more** reveals additional entries. **Clear URL history** asks for confirmation and clears both modes, including while tracking is off. If tracking remains on, subsequent visits start counting again. Clearing URL history does not clear keyword counts, profiles or Chrome's own history.
