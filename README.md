...
Choose a profile from **Profile** to view its details. Profiles are read-only by default. **Search keywords** filters both keyword groups immediately using case-insensitive partial matches; clearing it restores every row. Switching profiles resets search. Each keyword checkbox shows activity: checked means active, unchecked means inactive. In view mode it is disabled and labeled read-only; choose **Edit** to change activity.
